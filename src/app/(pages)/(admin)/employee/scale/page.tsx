...
export default function EmployeeScale() {
  ...
  useEffect(() => {}, [pagePosition, setPagePosition]);
  ...
  return (
    <>
      ...
      <section className="flex flex-col items-center gap-8 h-auto mt-[112px]">
        <h1 className="font-normal text-primary text-[50px] leading-[45px] mt-[50px] w-[95%]">
          {" "}
        ...
        <div className="flex gap-[25px] items-start justify-center w-11/12 h-fit">
          <div className="mt-[46px] gap-[25px] flex flex-col">
            {hoursList.map((h, idx) => {
              return (
                <span
                  key={idx}
                  className="font-medium text-primary text-[30px] h-[200px] flex items-center"
                >
                  {h}
                </span>
              );
          ...
          <div className="flex gap-[50px] overflow-hidden w-[70%] h-[950px] pb-5 justify-start items-center">
            {daysOfMonth.map((day, idx) =>
              format(day, "E", { locale: localePtBr }) === "sábado" ||
              format(day, "E", { locale: localePtBr }) === "domingo" ? (
                <></>
              ) : (
                <ul key={idx} className="flex flex-col day">
                  <span className="font-medium text-primary text-[30px] w-full text-center mb-[15px]">
                    {format(day, "E", { locale: localePtBr })
                      .charAt(0)
                      .toUpperCase()}
                    {format(day, "E", { locale: localePtBr }).slice(1, 3)}:{" "}
                    {format(day, "dd/MM")}
                  </span>
                  ...
                  <li className="flex flex-col gap-[25px]">
                    {hoursList.map((h, idx) => {
                      return (
                        <div
                          key={idx}
                          className="w-[200px] h-[200px] bg-[#d9d9d9] text-black flex justify-center items-center rounded-lg text-center"
                        >
                          ESDRINHAS CIRURGIAS
                        </div>
                      );
